        ...
        public async Task<JsonResult> ContactInquiry(Inquiry contactInquiryItems) 
        {
            ...
            if (contactInquiryItems != null) 
            {
                ...
                if (res.StatusCode.ToString().ToLower() == "created")
                {
                    ...
                    if(siteObj != null && !string.IsNullOrEmpty(siteObj.LeadFromEmail)) sendLeadEmail = await EmailUtil.SendEmail("support@VehiclesDemo.com", siteObj.LeadFromEmail, "New Lead Notification", stringBuilder1.ToString(), "Vehicle Demo Support");
                    sendClientEmail = await EmailUtil.SendEmail("support@VehiclesDemo.com", contactInquiryItems.email, "General Inquiry", stringBuilder.ToString(), "Vehicle Demo Support");
